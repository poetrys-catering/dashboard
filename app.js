...
const app = {
  ...
  init() {
    ...
    if (this.data.currentUser) {
      ...
      const defaultPage = this.getDefaultLandingPage(this.data.currentUser);
  ...
  applyRoles() {
    ...
    // Resolve allowed menus
    let allowed = [];
    if (user.allowedmenus) {
      allowed = user.allowedmenus.split(',').map(m => m.trim().toLowerCase());
    } else {
      // Fallback to default role permissions
      if (role === 'super admin') {
        allowed = ['dashboard', 'invoices', 'schedules', 'menus', 'users', 'reports', 'settings'];
      } else if (role === 'admin') {
        allowed = ['dashboard', 'invoices', 'schedules', 'menus', 'reports', 'settings'];
      } else if (role === 'staff') {
        allowed = ['dashboard', 'schedules'];
      } else {
        allowed = ['schedules'];
      }
    }

    // 1. Update sidebar menu items visibility
    ...
    // 2. Hide general section title, management header and dividers based on menu subsets
    const generalTitle = document.querySelector('.sidebar-nav .nav-title[data-lang-key="general"]');
    const mgmtTitle = document.querySelector('.sidebar-nav .nav-title[data-lang-key="management"]');
    const divider = document.querySelector('.sidebar-nav .nav-divider');

    const hasGeneral = allowed.includes('dashboard') || allowed.includes('invoices') || allowed.includes('schedules');
    const hasManagement = allowed.includes('menus') || allowed.includes('users') || allowed.includes('reports') || allowed.includes('settings');

    if (generalTitle) {
      if (hasGeneral && !(allowed.includes('schedules') && allowed.length === 1)) {
    ...
    if (mgmtTitle) {
      if (hasManagement) mgmtTitle.classList.remove('hidden');
      else mgmtTitle.classList.add('hidden');
    }
    if (divider) {
      if (hasManagement) divider.classList.remove('hidden');
      else divider.classList.add('hidden');
    }

    // Additional: Toggle dashboard link based on allowed list
    const dashLink = document.querySelector('.sidebar-nav a[data-page="dashboard"]');
    if (dashLink) {
    ...
    // 3. Hide/show admin-only / super-admin-only elements in general page layouts (excluding sidebar/dashboard components already managed)
    const adminElements = document.querySelectorAll('.admin-only:not(.nav-link):not(.nav-title):not(.nav-divider):not(.stats-grid):not(.hero-stat-card):not(.widget)');
    adminElements.forEach(el => {
      // Check if this element is an action button or container related to invoices, menus, or settings
      const isInvoiceAction = el.getAttribute('onclick')?.includes("'invoice'") || el.getAttribute('onclick')?.includes('create-invoice');
      const isMenuAction = el.getAttribute('onclick')?.includes("'menu'");
      const isGlobalSearch = el.classList.contains('header-search-container');
      
      if (isInvoiceAction) {
        if (allowed.includes('invoices')) el.classList.remove('hidden');
        else el.classList.add('hidden');
      } else if (isMenuAction) {
        if (allowed.includes('menus')) el.classList.remove('hidden');
        else el.classList.add('hidden');
      } else if (isGlobalSearch) {
        if (allowed.includes('menus') || allowed.includes('invoices')) el.classList.remove('hidden');
        else el.classList.add('hidden');
      } else {
    ...
    const superAdminElements = document.querySelectorAll('.super-admin-only:not(.nav-link):not(.nav-title):not(.nav-divider)');
    ...
    try {
      ...
      if (this.data.apiUrl) {
        ...
        if (json.status === 'success') {
          ...
          this.checkAuth();
          await this.loadData();
          const defaultPage = this.getDefaultLandingPage(json.user);
          this.navigate(defaultPage);
        } else {
  ...
  getDefaultLandingPage(user) {
    ...
    if (role === 'user' && allowed.includes('schedules') && !allowed.includes('dashboard')) {
      defaultPage = 'schedules';
    } else if (!allowed.includes(defaultPage)) {
      defaultPage = allowed.length > 0 ? allowed[0] : 'schedules';
    }
    return defaultPage;
  },
  ...
  navigate(page, silent = false) {
    ...
    if (user) {
      ...
      let checkPage = page;
      if (page === 'create-invoice') checkPage = 'invoices';

      if (!allowed.includes(checkPage)) {
        if (!silent) {
          this.showAlert('Anda tidak memiliki akses ke modul ini.', 'error', 'Akses Ditolak');
        }
        const fallback = allowed.length > 0 ? allowed[0] : 'schedules';
        this.navigate(fallback, silent);
        return;
      }
    }
    ...
    if (!this.data.apiUrl) {
      ...
      try {
        ...
        if (json.status === 'success' && json.data) {
          ...
          // Dynamically synchronize the requesting user's profile and permissions
          if (json.data.currentUser) {
            this.data.currentUser = json.data.currentUser;
      ...
      try {
        ...
        if (json.status === 'success' && json.data) {
          ...
          if (index > -1) {
            ...
            // Mapped on server directly
            items[index] = { ...items[index], ...serverData };
            this.data[target] = items;
            localStorage.setItem(target, JSON.stringify(items));
          }
        }
      ...
      try {
        ...
        if (json.status === 'success' && json.data) {
          ...
          if (idx > -1) {
            const serverData = { ...json.data };
            // Mapped on server directly
            items[idx] = { ...items[idx], ...serverData };
            this.data[target] = items;
            localStorage.setItem(target, JSON.stringify(items));
          }
        }
  ...
  renderInvoices() {
    ...
    let filtered = this.data.invoices.filter(inv => {
      ...
      if (q) {
        ...
        const vendorMatch = inv.vendor && String(inv.vendor).toLowerCase().includes(q);
        if (!numMatch && !nameMatch && !locMatch && !vendorMatch) return false;
      }
      ...
      pageItems.forEach(inv => {
        ...
        table.innerHTML += `
          <tr>
            ...
            <td>${this.escapeHTML(inv.customerName || '-')}</td>
            <td>${this.escapeHTML(inv.cateringLocation || '-')}</td>
            <td>${this.escapeHTML(inv.vendor || '-')}</td>
  ...
  populatePDFTemplate(inv) {
    ...
    if (inv.status === 'Draft') {
      ...
      watermark.style.color = 'rgba(100,116,139,0.18)';
      watermark.style.borderColor = 'rgba(100,116,139,0.18)';
    } else if(inv.status === 'Lunas' || remaining <= 0) {
    ...
    items.forEach(item => {
      let nameHtml = this.escapeHTML(item.name);
      if (item.memo) {
        nameHtml += `<div style="font-size: 11px; color: #64748b; font-style: italic; margin-top: 3px;">Catatan: ${this.escapeHTML(item.memo)}</div>`;
      }
      pdfTbody.innerHTML += `
        <tr>
          <td style="padding: 12px 15px; border-bottom: 1px solid #f1f5f9; color: #0f172a; font-weight: 500;">${nameHtml}</td>
    ...
    if (pdfDiscRow) {
      if (discType !== 'none' && discAmt > 0) {
        ...
        pdfDiscRow.style.display = 'table-row';
      } else {
    ...
    if (pdfAddFeeRow) {
      if (addFee > 0) {
        ...
        pdfAddFeeRow.style.display = 'table-row';
      } else {
    ...
    // Render Dynamic Payment History inside PDF Summary Box (Table row structure)
  ...
  async sendWhatsAppReminder(id) {
    ...
    const phone = this.formatPhoneForWA(inv.vendor);
    if (!phone) {
  ...
  updateNotifications() {
    ...
    // 1. Gather due/overdue invoices (Only for admins/super admins)
    const role = this.data.currentUser ? this.data.currentUser.role : 'user';
  ...
  updateEditInvoiceItemsTable() {
    ...
    this.editInvoiceState.items.forEach((item, i) => {
      ...
      tbody.innerHTML += `
        <tr>
          <td class="col-item" style="padding: 8px;"><strong style="color:var(--color-text);">${this.escapeHTML(item.name)}</strong></td>
          <td class="col-price" style="padding: 8px;">
            <input type="text" inputmode="numeric" value="${this.formatNumberToIndonesian(item.price)}"
              oninput="this.value=app.formatNumberToIndonesian(app.parseIndonesianToNumber(this.value)); app.updateEditInvoiceItemPrice(${i}, this.value)"
              style="width:100%; text-align:right; padding:4px 8px; border-radius:6px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text); font-size:13px; font-weight:600;">
          </td>
          <td class="col-qty" style="padding: 8px;">
            <input type="text" inputmode="numeric" maxlength="4" value="${item.qty}"
              ...
              style="width:100%; text-align:center; padding:4px 8px; border-radius:6px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text); font-size:13px; font-weight:600;">
          </td>
          <td class="col-unit" style="text-align: center; padding: 8px;"><span class="badge" style="background: rgba(255,255,255,0.08); padding: 2px 6px; border-radius: 4px; font-weight: 500;">${this.escapeHTML(item.unit || 'pax')}</span></td>
          <td class="col-subtotal" style="text-align: right; padding: 8px; font-weight: 600;" id="edit-inv-item-sub-${i}">${this.formatCurrency(item.subtotal)}</td>
          <td class="col-memo" style="padding: 8px;">
            <input type="text" value="${this.escapeHTML(item.memo || '')}"
              oninput="app.updateEditInvoiceItemMemo(${i}, this.value)"
              placeholder="Catatan..."
              style="width:100%; padding:4px 8px; border-radius:6px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text); font-size:13px;">
          </td>
          <td class="col-action" style="text-align: center; padding: 8px;"><button type="button" class="btn btn-danger btn-sm" style="padding: 4px 8px;" onclick="app.removeEditInvoiceItem(${i})"><i data-lucide="trash-2" style="width:14px; height: 14px;"></i></button></td>
        </tr>
      `;
    });
  ...
  updateInvoiceItemPrice(index, val) {
    const price = this.parseIndonesianToNumber(val);
    const item = this.currentInvoice.items[index];
    if (!item) return;
    item.price = price;
  ...
  updateInvoiceItemMemo(index, val) {
    const item = this.currentInvoice.items[index];
  ...
  updateEditInvoiceItemPrice(index, val) {
    const price = this.parseIndonesianToNumber(val);
    const item = this.editInvoiceState.items[index];
    if (!item) return;
    item.price = price;
  ...
  updateEditInvoiceItemMemo(index, val) {
    const item = this.editInvoiceState.items[index];
    if (item) item.memo = val;
  },
  ...
  updateInvoiceItemsTable() {
    ...
    this.currentInvoice.items.forEach((item, i) => {
      ...
      tbody.innerHTML += `
        <tr>
          <td class="col-item"><strong style="color:var(--color-text);">${this.escapeHTML(item.name)}</strong></td>
          <td class="col-price">
            <input type="text" inputmode="numeric" value="${this.formatNumberToIndonesian(item.price)}"
              oninput="this.value=app.formatNumberToIndonesian(app.parseIndonesianToNumber(this.value)); app.updateInvoiceItemPrice(${i}, this.value)"
              style="width:100%; text-align:right; padding:4px 8px; border-radius:6px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text); font-size:13px; font-weight:600;">
          </td>
          <td class="col-qty">
            <input type="text" inputmode="numeric" maxlength="4" value="${item.qty}"
          ...
          <td class="col-unit" style="text-align:center;"><span class="badge" style="background: rgba(255,255,255,0.08); padding: 4px 8px; border-radius: 4px; font-weight: 500;">${this.escapeHTML(item.unit || 'pax')}</span></td>
          <td class="col-subtotal" id="inv-item-sub-${i}" style="font-weight:bold; text-align:right;">${this.formatCurrency(item.subtotal)}</td>
          <td class="col-memo">
            <input type="text" value="${this.escapeHTML(item.memo || '')}"
              oninput="app.updateInvoiceItemMemo(${i}, this.value)"
              placeholder="Catatan..."
              style="width:100%; padding:4px 8px; border-radius:6px; border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text); font-size:13px;">
          </td>
          <td class="col-action" style="text-align:center;"><button type="button" class="btn btn-danger btn-sm" onclick="app.removeInvoiceItem(${i})"><i data-lucide="trash-2" style="width:16px;"></i></button></td>
        </tr>
      `;
    });
  ...
  async saveInvoice(isDraft = false) {
    ...
    const vendorVal = document.getElementById('invVendor').value.trim();
  ...
  generatePDF(fileName) {
    try {
      ...
      this.showLoading(true, 'Membuka jendela cetak...');

      // Open a blank window
      const printWin = window.open('', '_blank');
      if (!printWin) {
        this.showLoading(false);
        this.showAlert('Gagal membuka jendela cetak. Pastikan pop-up browser diperbolehkan.', 'warning');
        return;
      }

      // Build HTML document with app stylesheets and our specific Arial print styling
          ...
          <head>
            ...
            <style>
              ...
              @media print {
                ...
                .pdf-container { padding: 30px !important; }
              }
            </style>
          </head>
          <body>
      ...
      container.innerHTML += `
        <div class="user-card-item${isLast ? ' user-card-last' : ''}">
          ...
          <div class="user-card-actions">
            <button class="btn btn-secondary btn-sm" onclick="app.openUserPermissionsModal('${u.id}')" title="Atur Akses Menu" style="background: rgba(14, 165, 233, 0.1); color: #0ea5e9; border: 1px solid rgba(14, 165, 233, 0.2);">
              <i data-lucide="key" style="width:14px; height:14px;"></i> Akses
            </button>
  ...
  openUserPermissionsModal(id) {
    const u = this.data.users.find(x => x.id == id);
    if (u) {
      document.getElementById('permModalUserId').value = u.id;
      document.getElementById('permModalUserDisplayName').textContent = u.name || u.username;
      document.getElementById('permModalUsername').textContent = u.username;
      document.getElementById('permModalRole').textContent = u.role.toUpperCase();

      // Reset checkboxes
      const menuCheckboxes = [
        { id: 'permCheckboxDashboard', value: 'dashboard' },
        { id: 'permCheckboxInvoices', value: 'invoices' },
        { id: 'permCheckboxSchedules', value: 'schedules' },
        { id: 'permCheckboxMenus', value: 'menus' },
        { id: 'permCheckboxUsers', value: 'users' },
        { id: 'permCheckboxReports', value: 'reports' },
        { id: 'permCheckboxSettings', value: 'settings' }
      ];

      let allowed = [];
      if (u.allowedmenus) {
        allowed = u.allowedmenus.split(',').map(m => m.trim().toLowerCase());
      } else {
        // Pre-check checkboxes based on default role permissions if allowedmenus is empty
      ...
      menuCheckboxes.forEach(cb => {
        const checkboxEl = document.getElementById(cb.id);
        if (checkboxEl) {
          checkboxEl.checked = allowed.includes(cb.value);
        }
      });

      // Reload icon labels for modal info alert
      const modalAlertIcon = document.querySelector('#userPermissionsModal [data-lucide]');
      if (modalAlertIcon) lucide.createIcons({ nodes: [modalAlertIcon] });
  ...
  async saveUserPermissions(e) {
    ...
    const id = document.getElementById('permModalUserId').value;
    const u = this.data.users.find(x => x.id == id);
    if (!u) {
      this.showLoading(false);
      this.showAlert("User tidak ditemukan.", "error");
      return;
    }
    ...
    const allowedMenusArray = [];
    menuCheckboxes.forEach(cb => {
      const checkboxEl = document.getElementById(cb.id);
      if (checkboxEl && checkboxEl.checked) {
        allowedMenusArray.push(cb.value);
      }
    });

    const payload = {
      ...u,
      allowedmenus: allowedMenusArray.join(',')
    };

    try {
      await this.updateRow('Users', payload);

      // Update local storage and app state immediately if the super admin modified their own permissions
      if (this.data.currentUser && this.data.currentUser.id == id) {
        this.data.currentUser.allowedmenus = payload.allowedmenus;
        localStorage.setItem('currentUser', JSON.stringify(this.data.currentUser));
        this.applyRoles();
      }

      this.closeModal('userPermissionsModal');
      this.showAlert("Hak akses menu berhasil diperbarui!", "success");
      this.renderUsers();
    } catch (err) {
      console.error(err);
      this.showAlert("Gagal menyimpan hak akses ke spreadsheet: " + err.message, "error");
    } finally {
      this.showLoading(false);
    }
  },
  ...
  renderReports() {
    ...
    if (tbody) {
      ...
      const vendorMap = {};
      filteredInvoices.forEach(inv => {
        const name = (inv.vendor || 'Vendor Tidak Diketahui').trim();
        if (!vendorMap[name]) {
          vendorMap[name] = { name: name, eventCount: 0, totalAmount: 0, totalRemaining: 0 };
        }
        
        const paid = Number(inv.paidAmount) || (inv.status === 'Lunas' ? Number(inv.totalAmount) : 0);
        const remaining = Math.max(0, Number(inv.totalAmount) - paid);
        
        vendorMap[name].eventCount += 1;
        vendorMap[name].totalAmount += Number(inv.totalAmount);
        vendorMap[name].totalRemaining += remaining;
      });
      ...
      if (clientTbody) {
        clientTbody.innerHTML = '';
         if (sortedVendors.length === 0) {
           clientTbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 40px; color: var(--color-text-muted);">Belum ada data vendor.</td></tr>`;
         } else {
           sortedVendors.forEach((client, idx) => {
  ...
  exportReportToPDF() {
    ...
    try {
      ...
      filteredInvoices.forEach(inv => {
        ...
        vendorMap[vName].eventCount += 1;
        vendorMap[vName].totalAmount += Number(inv.totalAmount);
        vendorMap[vName].totalPaid += paid;
        vendorMap[vName].totalRemaining += remaining;
      });
      ...
      if (clientTbody) {
        clientTbody.innerHTML = '';
        if (sortedVendors.length === 0) {
          clientTbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 20px; color: #94a3b8;">Belum ada data vendor.</td></tr>`;
        } else {
          sortedVendors.forEach((client, idx) => {
            clientTbody.innerHTML += `
              <tr style="border-bottom: 1px solid #e2e8f0;">
  ...
  confirmPrintReport() {
    try {
      ...
      const periodText = document.getElementById('pdfReportPeriod').textContent;

      printWin.document.write(`
        <!DOCTYPE html>
        <html>
          <head>
            <title>Laporan Pendapatan - ${periodText}</title>
            <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">
            <style>
              body { 
                background: #ffffff !important; 
      ...
      this.showAlert('Laporan berhasil dikirim ke printer!', 'success');
    } catch (err) {
      console.error(err);
      this.showLoading(false);
      this.showAlert('Terjadi kesalahan saat mencetak laporan: ' + err.message, 'error');
    }
  },
  ...
  confirmMenuSelection() {
    ...
    const existingItems = target === 'create' ? (this.currentInvoice.items || []) : (this.editInvoiceState.items || []);
    let targetArray = [];
    ...
    Object.keys(selected).forEach(menuId => {
      ...
      if (itemState.checked) {
        const menu = this.data.menus.find(m => m.id == menuId);
        if (menu) {
          const existing = existingItems.find(it => it.menuId == menuId || it.name === menu.name);
          const price = existing && existing.price !== undefined ? Number(existing.price) : (Number(menu.price) || 0);
          const memo = existing ? (existing.memo || '') : '';
          const qty = Number(itemState.qty) || 1;
          targetArray.push({
            menuId: menu.id,
            name: menu.name,
            price: price,
